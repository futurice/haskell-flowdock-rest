...
performRequest :: (MonadIO m, MonadThrow m, FromJSON a)
               => Manager -> AuthToken -> Tagged a Request -> m a
...
messages :: (MonadIO m, MonadThrow m)
         => Manager
         -> AuthToken
         ...
         -> MessageOptions
         -> m [Message]
messages mgr token org fl opts =
  performRequest mgr token =<< messagesRequest org fl opts
...
flowUsers :: (MonadIO m, MonadThrow m)
          => Manager
          -> AuthToken
          -> ParamName Organisation
          -> ParamName Flow
          -> m [User]
flowUsers mgr token org fl =
  performRequest mgr token =<< flowUsersRequest org fl

organisationUsers :: (MonadIO m, MonadThrow m)
                  => Manager
                  -> AuthToken
                  -> ParamName Organisation
                  -> m [User]
organisationUsers mgr token org =
  performRequest mgr token =<< organisationUsersRequest org

-- Organisations

organisations :: (MonadIO m, MonadThrow m)
              => Manager -> AuthToken -> m [Organisation]
organisations mgr token =
  performRequest mgr token =<< organisationsRequest

organisation :: (MonadIO m, MonadThrow m)
             => Manager
             -> AuthToken
             -> ParamName Organisation
             -> m Organisation
organisation mgr token org =
  performRequest mgr token =<< organisationRequest org
